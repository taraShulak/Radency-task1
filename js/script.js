...
function createForm (event, category, content) {  
  if (formVisio == false){
...
class ListNote{
  ...
  correctNote(event){
    if (formVisio == false){
      ...
      createForm(event, category, content );
    }
  }  
  ...
  createNote(event){
      ...
      if(globalNoteIndex < noteArray.length) {
        ...
        if (inputCategory == undefined){
          inputCategory = noteArray[globalNoteIndex].category;
        }
        ...
        noteArray[globalNoteIndex].category = inputCategory;
        noteArray[globalNoteIndex].other = date.toLocaleDateString();
        globalNoteParent.querySelector('.others').innerHTML = date.toLocaleDateString();        
        globalNoteIndex = undefined;
        inputCategory = undefined;
      } else {                  
            if(inputCategory == undefined){ inputCategory = 'random'}
            const note = {
    ...
    if (chouseCategoryVisio == false){
      ...
      const text = `
              <div class="category-for-delete">  
                ...
                <div class="chouse-random action-item"  data-action="selectCat">random</div>
              </div>  
              `
